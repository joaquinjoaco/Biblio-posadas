...
export const ReassignModal: React.FC<ReassignModalProps> = ({
    ...
    const onSubmit = async (data: OrderFormValues) => {
        try {
            ...
            router.refresh();
            toast.success(`Pedido reasignado a ${currentDriver?.name}`);
        } catch (error) {
    ...
    return (
        <Modal
            title="Reasigna un repartidor"
            description="El pedido pasará asignarse al repartidor seleccionado."
            isOpen={isOpen}
            onClose={onClose}
        >
            <Form {...form}>
